...
def load_input():
	input_image = cv2.imread('eyes.png')
	augment_image = cv2.imread('specs.png')

	input_image = cv2.resize(input_image, (308,175),interpolation=cv2.INTER_AREA)
	augment_image = cv2.resize(augment_image, (308,175))
...
# Fuctions for computing matches between the train and query descriptors
